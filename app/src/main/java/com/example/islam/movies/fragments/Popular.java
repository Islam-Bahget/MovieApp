...
public class Popular extends Fragment implements DataView, ReAdapter.OnMovieClickListener, ReAdapter.OnLongItemListene {
    ...
    private void loadFirstPage() {
        ...
        presenter.getMovies(currentPage, "popular", Const.MOVIES);
    }
    ...
    public void onItem(int position) {
        ...
        selected.add(movies.get(position));
        ...
        switch (item.getItemId()) {
            case R.id.add_all:
                ...
                for (int i = 0; i < selected.size(); i++) {
                    Movie movie = selected.get(i);
                    DataBaseSingleton.laterDao(getContext()).insertLater(new WatchLaterEntity(movie.getId(), Const.MOVIE_TYPE, movie.getTitle(), movie.getBackdropPath()));
                }
